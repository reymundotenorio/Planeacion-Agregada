 ...
 */
public class FuerzaNDataSource implements JRDataSource {

    private List<FuerzaNPOJO> FuerzaN = new ArrayList<FuerzaNPOJO>();
    ...
    public Object getFieldValue(JRField jrf) throws JRException {
        ...
        if ("Semana".equals(jrf.getName())) {
            valor = FuerzaN.get(iterator).getSemana();
        }
        if ("Prod_req".equals(jrf.getName())) {

            valor = FuerzaN.get(iterator).getProd_req();
        }
        if ("Hrs_req".equals(jrf.getName())) {

            valor = FuerzaN.get(iterator).getHrs_req();
        }

        if ("Hrs_disp".equals(jrf.getName())) {

            valor = FuerzaN.get(iterator).getHrs_disp();
        }

        if ("Hrs_disp_N".equals(jrf.getName())) {

            valor = FuerzaN.get(iterator).getHrs_disp_N();
        }

        if ("Unids_prod".equals(jrf.getName())) {

            valor = FuerzaN.get(iterator).getUnids_prod();
        }

        if ("Unids_falt".equals(jrf.getName())) {

            valor = FuerzaN.get(iterator).getUnids_falt();
        }

        if ("Unids_sobr".equals(jrf.getName())) {

            valor = FuerzaN.get(iterator).getUnids_sobr();
        }

        if ("Hrs_E".equals(jrf.getName())) {

            valor = FuerzaN.get(iterator).getHrs_E();
        }
        if ("Hrs_O".equals(jrf.getName())) {

            valor = FuerzaN.get(iterator).getHrs_O();
        }

        if ("Cost_OS".equals(jrf.getName())) {

            valor = FuerzaN.get(iterator).getCost_OS();
        }
        if ("Cost_mant".equals(jrf.getName())) {

            valor = FuerzaN.get(iterator).getCost_mant();
        }

        if ("Cost_normal".equals(jrf.getName())) {

            valor = FuerzaN.get(iterator).getCost_normal();
        }
        if ("Cost_extra".equals(jrf.getName())) {

            valor = FuerzaN.get(iterator).getCost_extra();
        }

        if ("Cost_total".equals(jrf.getName())) {

            valor = FuerzaN.get(iterator).getCost_total();
        }
...
//        System.out.println("INTERTOR: " + iterator);
    ...
    public void addValor(FuerzaNPOJO A) {
        this.FuerzaN.add(A);
    }
